...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        printf(" 5) Reload dataset\n");
        printf(" 6) Exit\n");
        printf("Choose: ");
        if (!fgets(input, sizeof(input), stdin)) break;
        trim_newline(input);
        if (input[0] == '6' || input[0] == '\0') {
            printf("Goodbye!\n");
            break;
        }
        ...
        switch (input[0]) {
            ...
            case '5':
                ...
                if (fgets(input, sizeof(input), stdin)) {
                    trim_newline(input);
                    if (input[0] != '\0') {
                        if (reload_dataset(&db, &title_index, input)) {
                            printf("Reloaded dataset successfully.\n");
                        } else {
                            printf("Failed to reload dataset. Database remains unchanged.\n");
                        }
                    }
                }
                break;
